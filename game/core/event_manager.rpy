init -100 python:
    ...
    class Event(object):
        ...
        def __init__(self, name, triggers, conditions = [], *args, **kwargs):
            ...
            # if priority is given as integer, convert it to string (to be evaluated later)
            ...
            self.weekend_auto = kwargs.get('weekend_auto', False)
            self.weekend_room = kwargs.get('weekend_room', None)
        ...
        def check(self, already_passed):
            ...
            # Don't play weekend events
            if self.group == "ruler_event" and self.triggers == "week_end":
                return True
        ...
        def check_weekend_event(self, room, only_manual):
            if not room:
                return True

            if room in store.room_event_wait and store.room_event_wait[room] - week >= 0:
                return False

            if self.weekend_room != room:
                return False

            if self.weekend_auto and only_manual:
                return False
            elif not self.weekend_auto and not only_manual:
                return False
    ...
    class EventManager(object):
        ...
        def choose_events(self, trigger, room=None, only_manual=False):
            ...
            for group in event_groups:
                ...
                for ev in event_groups[group]:
                    ...
                    if not ev.check(self.candidates):
                        continue
                    # check weekend event conditions
                    if not ev.check_weekend_event(room, only_manual):
                        continue
        ...
        def mark_event_seen(self, ev_name):
            if ev_name not in all_events_data:
                return None

            all_events_data[ev_name]['run_count'] += 1

        def choose_room_event(self, room):
            self.choose_events("week_end", room)

            if len(self.events) == 0:
                return None

            room_event_wait[room] = week + 3
            return self.get_event()

        def get_all_room_events(self, room):
